...
`);
   ...
   */
  async updateMilestone(milestone, status) {
...
async function main() {
  ...
  const vaultRoot = path2.join(process.cwd(), "vault");
  const templatesPath = path2.join(__dirname, "../references");
  ...
  switch (command) {
    ...
    case "milestone":
      ...
      await manager.updateMilestone(milestone, status);
